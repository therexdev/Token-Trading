...
export function CreateLaunchPage() {
  ...
  const validation = useMemo((): string | null => {
    if (source === "mint") {
      if (authMethod === "bio") return "Connect Google or Kondor to mint a new token";
  ...
  }, [authMethod, source, mintName, mintSymbol, mintSupply, formToken, tokenBalance, forSale, locked, startAt, endAt, unlockAt, mode, price, softCap, hardCap, liqEnabled, liqPercent, liqTokens, lpUnlockAt]);
        ...
        <div className="mb-4 rounded-lg border border-ink-700 bg-ink-900 p-4">
          ...
          <div className="mb-3 flex gap-1 rounded-md bg-ink-950 p-1">
            ...
            <button
              ...
              disabled={!authConfig?.tokenLaunch || authMethod === "bio"}
              title={
                authMethod === "bio"
                  ? "Connect Google or Kondor to mint a new token"
          ...
          {authMethod === "bio" && (
            <p className="mb-3 text-xs text-ink-300">
              KOIN Vault can launch a token you already hold. Minting a new token, uploading a logo, and saving project links currently require Google or Kondor.
            </p>
          )}
          {source === "existing" ? (
